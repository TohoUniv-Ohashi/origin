...
public class TerritoryEvaluator {
	...
	public EvaluatedResult evaluateTerritory(PointState[][] pointStates) {
		...
		List<int[]> blackTerritory = new ArrayList<>();
		List<int[]> whiteTerritory = new ArrayList<>();
		...
		for (int i = 0; i < whiteStones.size(); i++) {
			...
			// arrayList更新
			updateList(result, blackTerritory, whiteTerritory, undefined);
		}
		...
		List<int[]> toN_point = new ArrayList<>();
		// 黒地に隣接する白地を取得
		for (int i = 0; i < blackTerritory.size(); i++) {
			int[] point = blackTerritory.get(i);
			List<int[]> w_point = onSide(result, point[0], point[1], WHITE_TERRITORY, WHITE_TERRITORY);
			if (w_point.size() > 0) {
				toN_point.add(point);
			}
		}
		...
		for (int i = 0; i < whiteTerritory.size(); i++) {
			int[] point = whiteTerritory.get(i);
			List<int[]> b_point = onSide(result, point[0], point[1], BLACK_TERRITORY, BLACK_TERRITORY);
			if (b_point.size() > 0) {
				toN_point.add(point);
			}
		}
		...
		for (int i = 0; i < toN_point.size(); i++) {
			...
			List<int[]> chenge_Point = new ArrayList<>();
		...
		/*
		 * チェック用 //6:最後に仮白地と仮黒地を数え上げ、それぞれの数を白地と黒地の数とする
		 * System.out.println("計算した黒地："+blackTerritory.size());
		 * System.out.println("計算した白地："+whiteTerritory.size()); PointState[][] board =
		 * result; for(int y=0;y<board.length;y++) { for(int x=0;x<board[0].length;x++)
		 * { if(x>0) { System.out.print(":"); } System.out.print(board[y][x]); }
	...
	// 変換した座標のArrayListを返す
	private List<int[]> onSide(PointState[][] result, int x, int y, PointState stateBefore, PointState stateAfter) {
		List<int[]> chenge_XYs = new ArrayList<>();
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				// 斜めと自分の場所を取り除く
				if (i != 0 && j != 0 || i == 0 && j == 0) {
	...
	// resultから各Listを更新する
	private void updateList(PointState[][] result, List<int[]> black, List<int[]> white, List<int[]> undefined) {
		List<int[]> chengePoints = new ArrayList<>();
		// 変更があった座標を抜き出す
		for (int i = 0; i < result.length; i++) {
			for (int j = 0; j < result[0].length; j++) {
				boolean on = false;
				...
				case BLACK_TERRITORY:
					for (int[] point : black) {
						if (Arrays.equals(point, checkPoint)) {
							on = true;
						}
					}
					break;
		...
		for (int i = 0; i < chengePoints.size(); i++) {
			...
			case BLACK_TERRITORY:
				black.add(chengePoints.get(i));
				white.removeIf(p -> Arrays.equals(p, point));
				undefined.removeIf(p -> Arrays.equals(p, point));
				break;
			case WHITE_TERRITORY:
				white.add(chengePoints.get(i));
				black.removeIf(p -> Arrays.equals(p, point));
				undefined.removeIf(p -> Arrays.equals(p, point));
				break;
			case UNDEFINED:
				undefined.add(chengePoints.get(i));
				white.removeIf(p -> Arrays.equals(p, point));
				black.removeIf(p -> Arrays.equals(p, point));
				break;
			default:
